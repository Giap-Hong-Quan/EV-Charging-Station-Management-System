...
const HeaderStaff = ({ isSidebarOpen, setIsSidebarOpen }) => {
  ...
  // ========================================
  // 🔥 LẤY TITLE THEO URL HIỆN TẠI
  // ========================================
  const getPageTitle = () => {
    const currentPath = location.pathname;

    const item = mainMenu.find(item => item.path === currentPath);
      ...
      <header className="bg-white border-b border-gray-200 shadow-sm">
        <div className="flex items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button onClick={() => setShowCreateSessionModal(true)} className="gap-2 bg-emerald-600 hover:bg-emerald-700 text-white">
              <Plus className="w-4 h-4" />
              Tạo phiên sạc
            </Button>
